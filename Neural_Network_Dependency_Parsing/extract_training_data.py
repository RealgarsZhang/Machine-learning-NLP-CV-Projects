...
class FeatureExtractor(object):
    ...
    def convert_index(self,arr,idx, words, pos, is_stack):
        if idx>len(arr):
            return self.word_vocab['<NULL>']
        elif pos[arr[-idx]] == 'CD':
            return self.word_vocab['<CD>']
        elif pos[arr[-idx]] == 'NNP':
            return self.word_vocab['<NNP>']
        elif arr[-idx]==0 or pos[arr[-idx]] == '<ROOT>' or words[arr[-idx]] == '<ROOT>' or (is_stack and idx == len(arr)):
            return self.word_vocab['<ROOT>']
        elif pos[arr[-idx]] == '<UNK>' or words[arr[-idx]]=='<UNK>' or words[arr[-idx]] not in self.word_vocab:
            return self.word_vocab['<UNK>']
        else:
            return self.word_vocab[words[arr[-idx]]]
    ...
    def get_input_representation(self, words, pos, state):
        ...
        res = []
        for i in range(1,4):
            res.append(self.convert_index(state.stack,i,words,pos,True))
        for i in range(1,4):
            res.append(self.convert_index(state.buffer,i,words,pos,False))
    ...
    def get_output_representation(self, output_pair):  
        ...
        #print(output_pair,self.output_labels[output_pair])
        print ("Label:",res)
        return res
